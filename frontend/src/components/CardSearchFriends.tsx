...
const CardSearchFriends: React.FC = () => {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/profile/name/${searchQuery}`);
      
      // Filtra a lista de amigos para remover o objeto com _id igual ao id do contexto
      const filteredFriendsList = response.data.reduce((acc: FriendListInterface[], friend: FriendListInterface) => {
    ...
    <div className="w-[36rem] h-[37rem] bg-[#dddbffd0] rounded-xl p-8 flex flex-col items-center">
      ...
      <div className="w-full overflow-y-auto space-y-2 p-3">
        {friendsList.map(friend => (
          <div key={friend.profileId} className="bg-white flex mb-2 p-2 rounded-md items-center">
            <div className="w-14 h-14 min-w-14 min-h-14 max-w-14 max-h-14 flex items-center justify-center overflow-hidden rounded-full">
              <Image src={friend.profilePicture + ".png"} alt="profile picture" height={64} width={64} />
            </div>
            <div className="flex ml-5 w-full items-center">
              <h2 className="text-text text-lg w-full mr-4">{friend.profileName}</h2>
              <button onClick={() => handleAddFriend(friend._id)} className="text-blue-500 text-sm mr-4">
